...
const payOrder = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  if (!isValidObjectId(orderId)) return res.status(400).json({ message: "El orden ID no es valido" });

  const { data } = await axios.get<IPaypal.IPaypalOrderResponse>(`${process.env.PAYPAL_ORDERS_URL}/${transactionId}`, {
    headers: {
